...
- **rules/analyzers** apply the heuristics shipped in `dockeraudit/core.py`.
- **score** ranks by severity.
- **MCP server** (`dockeraudit mcp`) exposes `scan` for Cognis.Studio agents.
