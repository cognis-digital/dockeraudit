...
[![PyPI](https://img.shields.io/pypi/v/cognis-dockeraudit.svg)](https://pypi.org/project/cognis-dockeraudit/)
[![CI](https://github.com/cognis-digital/dockeraudit/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/dockeraudit/actions)
...
DOCKERAUDIT exists for one job — audit dockerfiles + image configs for security smells — and does it without a SaaS bill or heavyweight setup.
...
```bash
dockeraudit --version
dockeraudit scan .                      # scan the current project
dockeraudit scan . --format json
dockeraudit scan . --fail-on high       # non-zero exit for CI gates
dockeraudit mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **hadolint/dockle**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`dockeraudit` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
