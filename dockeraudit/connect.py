"""Native cognis-connect emit for dockeraudit — forward findings to any platform.

Maps dockeraudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    dockeraudit ... --format json | dockeraudit-emit --to stix
    dockeraudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a dockeraudit record toward the Finding contract before normalize().
